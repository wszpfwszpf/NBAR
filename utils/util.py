...
def load_events_npz(npz_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict[str, Any]]:
    """
    Load events via the unified NPZ reader.

    Returns
    -------
    t, x, y, p, meta
        Arrays and metadata from :func:`nbar.io.load_npz` with all extra
        per-event channels preserved inside ``meta``.
    """
...
def save_events_npz(
    ...
    """Save events using the unified NPZ writer."""
...
def clip_events_npz(in_npz, out_npz, t_start, t_end, assume_sorted=True):
    events = load_events_npz_stream(in_npz)
    t = events.t
    x = events.x
    y = events.y
    p = events.p
    ...
    meta_out = dict(events.meta or {})
    meta_out["src"] = in_npz
    meta_out["t_range"] = np.array([t_start, t_end], dtype=np.float64)
    meta_out["count"] = np.array([len(t_clip)], dtype=np.int64)

    save_events_npz(out_npz, t_clip, x_clip, y_clip, p_clip, meta=meta_out)
...
def load_events_npz2(npz_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray,
    ...
    """
    Load events and split metadata/extra channels using the unified loader.

    meta
        Lightweight metadata (scalars/small arrays).
    extra
        Per-event long channels such as ``w1``/``w2``.
    """

    events = load_events_npz_stream(npz_path)
    ...
    if events.meta:
        N = int(t.size)
        forced_meta = {"resolution", "time_unit", "src", "t_range", "count"}
        for key, val in events.meta.items():
            if key in forced_meta:
                meta[key] = val
                continue
    ...
    meta.setdefault("path", npz_path)
    meta.setdefault("num_events", int(t.size))
    meta.setdefault("t_start", float(t[0]) if t.size else None)
    meta.setdefault("t_end", float(t[-1]) if t.size else None)
    if meta.get("resolution") is not None:
...
def save_events_npz2(
    ...
    *,
    ...
    """Save events with optional per-event channels via the unified writer."""

    t = np.asarray(t, dtype=np.float64)
    x = np.asarray(x, dtype=np.int32)
    y = np.asarray(y, dtype=np.int32)
    p_pm = np.where(np.asarray(p, dtype=np.int8) > 0, 1, -1).astype(np.int8)
    ...
    meta_out.setdefault("resolution", np.asarray(default_resolution, dtype=np.int32))
    meta_out.setdefault("time_unit", default_time_unit)

    events = EventStream(t=t, x=x, y=y, p=p_pm, meta=meta_out)
    save_events_npz_stream(save_path, events)
