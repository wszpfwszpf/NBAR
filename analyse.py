...
STAGE = "s1"
NPZ_PATH = rf"outputs/{STAGE}/mvsec_clip_2s_{STAGE}.npz"   # ← 改成你的 npz 路径
...
def save_events_txpy_npz(save_path, t, x, y, p, meta=None):
    ...
    save_npz(save_path, events)



def load_events_from_npz(npz_path: str, W: int = 346, H: int = 260, normalize_time: bool = True):
    """
    Load events using the unified NPZ reader and return normalized arrays.
    """
    events = load_npz(npz_path)

    if events.meta and events.meta.get("resolution") is not None:
        res = events.meta["resolution"]
        W, H = int(res[0]), int(res[1])

    return normalize_events_txpy(events.t, events.x, events.y, events.p, W=W, H=H, normalize_time=normalize_time)
